// A primeira coisa que fazemos: verificamos se a extensão está ativa
chrome.storage.sync.get(["isExtensionActive"], function (storage) {
  ...
  // Se NÃO estiver ativa, não fazemos absolutamente nada.
  if (!isActive) {
    return;
  }

  // Se ESTIVER ATIVA, executamos todo o código da extensão
  // (Todo o seu código antigo do content.js vai aqui dentro)
  ...
  analisarEAtivar();
}); // Fim da verificação de 'isActive'
